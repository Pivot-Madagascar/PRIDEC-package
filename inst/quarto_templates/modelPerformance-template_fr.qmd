...
TESTING PROGRAMMATICALLY CONTROLLED PANEL CREATION

```{r function for programmatic }
tabset_invVar <- function(.x, .f, ...){
  nms <- toupper(names(.x))
  ...
  for (i in seq_along(.x)){
    ...
    cat("#| output: true", "\n\n", sep = "")
    #set up figure heights
    if(nms[i] == "NAIVE"){
      cat("#| fig-height: 6", "\n\n", sep = "")
      cat("#| out-height: 6in", "\n\n", sep = "")
      print(patchwork::wrap_plots(plot_counterfactual(.x[[i]]), nrow = 2))
    } else {
      cat("#| fig-height: ",length(.x[[i]]), "\n\n", sep = "")
      cat("#| out-height: ",length(.x[[i]]), "in", "\n\n", sep = "")
      print(patchwork::wrap_plots(plot_counterfactual(.x[[i]]), ncol = 3))
    }
